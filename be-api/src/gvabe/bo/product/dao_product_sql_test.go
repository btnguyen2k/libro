...
func initDaoSql(sqlc *prom.SqlConnect) ProductDao {
	if sqlc.GetDbFlavor() == prom.FlavorCosmosDb {
		return NewProductDaoCosmosdb(sqlc, testSqlTable, true)
	}
	return NewProductDaoSql(sqlc, testSqlTable, true)
}
...
func TestProductDaoSql_CreateGet(t *testing.T) {
	name := "TestProductDaoSql_CreateGet"
	urlMap := sqlGetUrlFromEnv()
	if len(urlMap) == 0 {
		t.Skipf("%s skipped", name)
	}
...
func TestProductDaoSql_CreateUpdateGet(t *testing.T) {
	name := "TestProductDaoSql_CreateUpdateGet"
	urlMap := sqlGetUrlFromEnv()
	if len(urlMap) == 0 {
		t.Skipf("%s skipped", name)
	}
...
func TestProductDaoSql_CreateDelete(t *testing.T) {
	name := "TestProductDaoSql_CreateDelete"
	urlMap := sqlGetUrlFromEnv()
	if len(urlMap) == 0 {
		t.Skipf("%s skipped", name)
	}
