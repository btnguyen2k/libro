...
func NewTopicDaoDynamodb(adc *prom.AwsDynamodbConnect, tableName string) TopicDao {
	baseDao := &BaseTopicDaoImpl{}
	spec := &henge.DynamodbDaoSpec{}
	baseDao.UniversalDao = henge.NewUniversalDaoDynamodb(adc, tableName, spec)
	dao := &DynamodbTopicDao{
		BaseTopicDaoImpl:     baseDao,
		tableName:            tableName,
		gsiProductIdPosition: "gsi_" + tableName + "_" + TopicFieldProductId + "_" + TopicFieldPosition,
	}
	return dao
}
