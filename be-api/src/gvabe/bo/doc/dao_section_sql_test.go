...
func sqlInitTable(sqlc *prom.SqlConnect, table string) error {
	...
	case prom.FlavorMySql:
		err = henge.InitMysqlTable(sqlc, table, map[string]string{SectionColAppId: "VARCHAR(32)"})
	case prom.FlavorPgSql:
		err = henge.InitPgsqlTable(sqlc, table, map[string]string{SectionColAppId: "VARCHAR(32)"})
	case prom.FlavorSqlite:
		err = henge.InitSqliteTable(sqlc, table, map[string]string{SectionColAppId: "VARCHAR(32)"})
	}
	return err
}
...
func initDaoSql(sqlc *prom.SqlConnect) SectionDao {
	if sqlc.GetDbFlavor() == prom.FlavorCosmosDb {
		return NewSectionDaoCosmosdb(sqlc, testSqlTable, true)
	}
	return NewSectionDaoSql(sqlc, testSqlTable, true)
}
...
func TestNewSectionDaoSql(t *testing.T) {
	name := "TestNewSectionDaoSql"
...
func TestSectionDaoSql_CreateGet(t *testing.T) {
	name := "TestSectionDaoSql_CreateGet"
	urlMap := sqlGetUrlFromEnv()
	if len(urlMap) == 0 {
		t.Skipf("%s skipped", name)
	}
...
func TestSectionDaoSql_CreateUpdateGet(t *testing.T) {
	name := "TestSectionDaoSql_CreateUpdateGet"
	urlMap := sqlGetUrlFromEnv()
	if len(urlMap) == 0 {
		t.Skipf("%s skipped", name)
	}
...
func TestSectionDaoSql_CreateDelete(t *testing.T) {
	name := "TestSectionDaoSql_CreateDelete"
	urlMap := sqlGetUrlFromEnv()
	if len(urlMap) == 0 {
		t.Skipf("%s skipped", name)
	}
...
func TestSectionDaoSql_GetAll(t *testing.T) {
	name := "TestSectionDaoSql_GetAll"
	urlMap := sqlGetUrlFromEnv()
	if len(urlMap) == 0 {
		t.Skipf("%s skipped", name)
	}
...
func TestSectionDaoSql_GetN(t *testing.T) {
	name := "TestSectionDaoSql_GetN"
	urlMap := sqlGetUrlFromEnv()
	if len(urlMap) == 0 {
		t.Skipf("%s skipped", name)
	}
