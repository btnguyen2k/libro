...
// NewExterClient creates a new Exter client instance.
...
// ExterClient is an Exter http-client.
// available since template-v0.2.0
...
// ExterResponse encapsulates a response from Exter API call.
// available since template-v0.2.0
...
// ExterToken encapsulates an Exter token data.
// available since template-v0.2.0
...
// available since template-v0.2.0
func createUserFromExterToken(exterToken *ExterToken) (*user.User, error) {
	if exterToken == nil {
		return nil, nil
	}
	...
	usr, err := userDao.Get(exterToken.UserId)
	if err != nil {
	...
	if usr == nil {
		...
		usr = user.NewUser(goapi.AppVersionNumber, exterToken.UserId, utils.UniqueId())
		displayName := exterToken.UserName
		if displayName == "" {
			displayName = usr.GetMaskId()
		}
		usr.SetDisplayName(displayName).SetAdmin(false)
