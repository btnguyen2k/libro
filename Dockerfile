...
FROM node:12-alpine AS builder_fe_admin
RUN mkdir /build
RUN apk add jq sed
COPY . /build
RUN cd /build \
...
FROM node:12-alpine AS builder_fe_tpl_coderdocs
RUN mkdir /build
RUN apk add jq sed
COPY . /build
RUN cd /build \
...
RUN mkdir -p /app/frontend/admin /app/frontend/tpl_coderdocs
COPY --from=builder_be /build/be-api/main /app/main
COPY --from=builder_be /build/be-api/config /app/config
COPY --from=builder_fe_admin /build/fe-admin/dist /app/frontend/admin
COPY --from=builder_fe_tpl_coderdocs /build/fe-tpl-coderdocs/dist /app/frontend/tpl_coderdocs
